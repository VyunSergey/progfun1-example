 ...
 */
 class ListsSuite {
  ...
  @Test def `sum of a few numbers (10pts)`: Unit = {
    ...
    assert(sum(List.range(0, 100)) == List.range(0, 100).sum)
  }

  @Test def `max of a few numbers (10pts)`: Unit = {
    try {
      max(List.empty[Int])
      Assert.fail("No exception: NoSuchElementException has been thrown")
    } catch {
      case e: NoSuchElementException => ()
    }
    assert(max(List(1)) == 1)
    assert(max(List(3, 7, 2)) == 7)
    assert(max(List.range(0, 100)) == List.range(0, 100).max)
  }
